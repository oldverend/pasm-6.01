...
org <число>          — режим сборки:
                          ...
                          109h  / 109h0   -> .exe/консоль (пока не реализовано)
...
class Instr:
    ...
    def _compute_size(self):
        ...
        if k == 'read_line':
            return 26
        if k == 'print_string':
            return 13 if self.reg == 'BX' else 15
        if k == 'cmp_str':
...
def compile_pasm(src: str):
    ...
    for lineno, rawfull in enumerate(raw_lines, 1):
        ...
        if headl == 'mov':
            ...
            if is_number_token(val):
                ...
                if num == 0x10:
                    ...
                    program.append(Instr('print_string', reg=reg))
                    continue
        ...
        if headl == 'st':
            ...
            program.append(Instr('read_line', reg=reg))
            continue
    ...
    if mode != 'qemu':
        raise PasmError("в этой версии компилятора реализован только режим QEMU (org 1000h)")

    # ---- "mov reg,10h0" уже превращён в print_string прямо при разборе
    #      (см. выше) — отдельный проход больше не нужен ------------------

    program.append(Instr('halt_loop'))

    # ---- address resolution (программа теперь всегда грузится вторым
    #      этапом, начиная с STAGE2_LOAD_ADDR — см. compile-time генерацию
    #      загрузчика в конце функции) ------------------------------------
    addr = STAGE2_LOAD_ADDR
    for ins in program:
        ins.addr = addr
        addr += ins.size
